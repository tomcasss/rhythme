...
export const API_ENDPOINTS = {
  ...
  GET_TIMELINE_POSTS: (userId) =>
    `${BASE_URL}/posts/get-timeline-posts/${userId}`,
  ...
  MARK_NOTIFICATION_AS_READ: (notifId) =>
  ...
  SPOTIFY_SEARCH: (query, type, limit) =>
    `${BASE_URL}/spotify/search?query=${encodeURIComponent(query)}&type=${
  ...
  SPOTIFY_CONNECTION_STATUS: (userId) =>
    `${BASE_URL}/spotify/connection-status/${userId}`,
  SPOTIFY_DISCONNECT: (userId) => `${BASE_URL}/spotify/disconnect/${userId}`,
  SPOTIFY_USER_PLAYLISTS: (userId, limit, offset) =>
    `${BASE_URL}/spotify/user/${userId}/playlists?limit=${limit || 20}&offset=${
      offset || 0
    }`,
  SPOTIFY_USER_SAVED_TRACKS: (userId, limit, offset) =>
    `${BASE_URL}/spotify/user/${userId}/saved-tracks?limit=${
      limit || 20
    }&offset=${offset || 0}`,
  SPOTIFY_USER_TOP_ARTISTS: (userId, limit, timeRange) =>
    `${BASE_URL}/spotify/user/${userId}/top-artists?limit=${
      limit || 20
    }&timeRange=${timeRange || "medium_term"}`,
  SPOTIFY_USER_TOP_TRACKS: (userId, limit, timeRange) =>
    `${BASE_URL}/spotify/user/${userId}/top-tracks?limit=${
      limit || 20
    }&timeRange=${timeRange || "medium_term"}`,
};
