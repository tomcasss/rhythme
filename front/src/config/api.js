...
export const API_ENDPOINTS = {
  ...
  FRIEND_RECOMMENDATIONS: (userId, limit) =>
    `${BASE_URL}/users/${userId}/recommendations/friends${limit ? `?limit=${limit}` : ""}`,
  REPORT_USER: (targetId) => `${BASE_URL}/users/${targetId}/report`,
  LIST_REPORTS: (status, limit) =>
    `${BASE_URL}/users/reports/all${status ? `?status=${status}` : ""}${limit ? `${status ? "&" : "?"}limit=${limit}` : ""}`,
  ...
  GET_RECOMMENDED_POSTS: (userId, limit) =>
    `${BASE_URL}/posts/recommended/${userId}${limit ? `?limit=${limit}` : ""}`,
  ...
  SPOTIFY_SEARCH: (query, type, limit) =>
    `${BASE_URL}/spotify/search?query=${encodeURIComponent(query)}&type=${type || "track,artist,playlist,album"}&limit=${limit || 20}`,
  ...
  SPOTIFY_USER_PLAYLISTS: (userId, limit, offset) =>
    `${BASE_URL}/spotify/user/${userId}/playlists?limit=${limit || 20}&offset=${offset || 0}`,
  SPOTIFY_USER_SAVED_TRACKS: (userId, limit, offset) =>
    `${BASE_URL}/spotify/user/${userId}/saved-tracks?limit=${limit || 20}&offset=${offset || 0}`,
  SPOTIFY_USER_TOP_ARTISTS: (userId, limit, timeRange) =>
    `${BASE_URL}/spotify/user/${userId}/top-artists?limit=${limit || 20}&timeRange=${timeRange || "medium_term"}`,
  SPOTIFY_USER_TOP_TRACKS: (userId, limit, timeRange) =>
    `${BASE_URL}/spotify/user/${userId}/top-tracks?limit=${limit || 20}&timeRange=${timeRange || "medium_term"}`,
};
